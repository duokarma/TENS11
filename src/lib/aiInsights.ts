...
export function parseAIQuery(query: string, ctx: AIQueryContext): string {
  ...
  const now = new Date();  // ── Monthly revenue trend (last 6 months) ────────────────────────────────
  ...
  if (q.includes('busiest day') || q.includes('best day') || q.includes('day of week') || q.includes('which day') || q.includes('most revenue')) {
  ...
  if (q.includes('top customer') || q.includes('best customer') || q.includes('highest spend') || q.includes('spent the most') || q.includes('spent most')) {
    const monthStart = q.includes('month') ? startOfMonth(now) : new Date(0);
